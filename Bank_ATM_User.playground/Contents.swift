...
//      1.1. Проверяется правильные ли: номер карты + пинкод
//      1.2. Банк выводит сообщение:
//          Здравствуйте Валерий Евгеньевич. Вы запросили баланс. Ваш баланс Х. Хорошего дня!

//    2. Снятие наличных с банковского депозита:
//      2.1. Проверяется правильные ли: номер карты + пинкод
//      2.2. Пользователь должен ввести какую то сумму денег, эта сумма не должна превышать его депозитный счет, т.е. должна проходить проверка по колличеству денег находящихся на депозите, и если запрашиваемая сумма не превышает то банкомат дает пользователю наличные
//      2.3. Банк выводит сообщение:
...
//      3.1. Проверяется правильные ли: номер карты + пинкод
//      3.2. Проверка наличия наличных. Можем ли столько внести? Сумма которую вносим не должна превышать имеющуюся наличность
//      3.3. Банк выводит сообщение:
...
//      4.1. Проверяется правильные ли: номер карты + пинкод
//      4.2. Проверяется корректно ли введен номер телефона
//      4.3. Уточняется от куда пополнить телефон: С депозита или с наличных
//      4.4. Банк выводит сообщение:
...
//ToDo: Для лучшей читаемости кода необходимо разобраться с неймингом: Переименовать все сложные для восприятия, неточные или двусмыслнные названия. (Осуществимо если начать переиминовывать названия переменных и методов данных в задании из начала)
...
//    Функции выводящие сообщения (своей внутренней логикой не обладают!)
  ...
  func showError(error: TextErrors) // Метод который отвечает за ошибки. Все ошики завернуты в один вenum.
 
//    Функции со своей внутренней логикой:
  func checkUserPhone(phone: String) -> Bool // Банк проверяет правильный прислали номер телефона, если неправилный то false
  ...
  func checkCurrentUser(userCardId: String, userCardPin: Int) -> Bool // Метод отвечает за проверку  номера карты и ПИН самого пользователя
